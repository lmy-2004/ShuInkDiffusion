...
def _resize_video(video: torch.Tensor, height: int, width: int) -> torch.Tensor:
    ...
    return trans(video.permute(0, 3, 1, 2)).permute(0, 2, 3, 1).contiguous()


def _load_mp4_video(input_path: str, height: int, width: int, max_frames: int) -> torch.Tensor:
    ...
    for start in range(0, len(frame_indices), 32):
        chunk_indices = frame_indices[start : start + 32]
        chunk = torch.from_numpy(reader.get_batch(chunk_indices).asnumpy()).to(dtype=torch.float32).div_(255)
        video_chunks.append(_resize_video(chunk, height, width))
...
def main(
    ...
    tiny_vae_path: Optional[str] = None,
    opt_unet: bool = False,
    fps: int = 16,
    ...
    tiny_vae_path : Optional[str], optional
        The local TinyVAE directory. If None, use `models/taesd`.
    ...
    if os.path.isdir(input):
        video = read_video_frames(input).to(dtype=torch.float32) / 255
    elif input.endswith(".mp4"):
        video = _load_mp4_video(input, height, width, max_frames)
        ...
        video = torch.from_numpy(np.array(video_frames)).to(dtype=torch.float32) / 255

    if not input.endswith(".mp4"):
        video = video[2:]
        video = _resize_video(video, height, width)
