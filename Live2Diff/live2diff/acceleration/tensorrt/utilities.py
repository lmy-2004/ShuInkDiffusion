...
class Engine:
    ...
    def build(
        ...
        if workspace_size > 0:
            pool_limits = {trt.MemoryPoolType.WORKSPACE: workspace_size}
            if tactic_dram_size > 0 and hasattr(trt.MemoryPoolType, "TACTIC_DRAM"):
                pool_limits[trt.MemoryPoolType.TACTIC_DRAM] = tactic_dram_size
            config_kwargs["memory_pool_limits"] = pool_limits
    ...
    def activate(self, reuse_device_memory=None, profiler=None):
    ...
    def allocate_buffers(self, shape_dict=None, device="cuda"):
        ...
        for idx in range(self.engine.num_io_tensors):
            ...
            binding_mode = self.engine.get_tensor_mode(binding)
            if binding_mode == TensorIOMode.INPUT:
...
def build_engine(
    ...
    if free_mem > 6 * GiB:
        ...
        max_tactic_dram = 0
    print(
        "TensorRT memory pools: "
        f"workspace={max_workspace_size / GiB:.2f} GiB, "
        f"tactic_dram={max_tactic_dram / GiB:.2f} GiB "
        f"(free: {free_mem / GiB:.2f} GiB)"
    )
    ...
    engine.build(
        ...
        tactic_dram_size=max_tactic_dram,
    )
...
def optimize_onnx(
    ...
    model_data.optimize(onnx_path, onnx_opt_path)
    # # onnx_opt_graph = model_data.optimize(onnx.load(onnx_path))
    # onnx_opt_graph = model_data.optimize(onnx_path)
