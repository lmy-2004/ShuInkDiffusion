...
class InflatedUNetDepth(BaseModel):
    def __init__(
        ...
        for tensor in kv_cache_list or []:
            shape = tensor.shape if hasattr(tensor, "shape") else tensor
            self.kv_cache_shapes.append(tuple(shape))
    ...
    def get_input_names(self):
        ...
        input_list += [f"kv_cache_{i}" for i in range(len(self.kv_cache_shapes))]
        input_list += ["pe_idx", "update_idx"]
        return input_list

    def get_output_names(self):
        output_list = ["latent"]
        output_list += [f"kv_cache_out_{i}" for i in range(len(self.kv_cache_shapes))]
        return output_list
    ...
    def get_input_profile(self, batch_size, image_height, image_width, static_batch, static_shape):
        ...
        for idx, shape in enumerate(self.kv_cache_shapes):
            input_profile[f"kv_cache_{idx}"] = [shape] * 3
    ...
    def get_sample_input(self, batch_size, image_height, image_width):
        ...
        kv_cache_inputs = [
            torch.zeros(shape, dtype=dtype, device=self.device) for shape in self.kv_cache_shapes
        ]
